...
// A turn may include live web research on the student's key.
...
export async function POST(req: Request): Promise<Response> {
  ...
  return sseTurnResponse(
    async (emit) =>
      (await runInterviewPrepTurn(key, ctx.store, appId, transcript, model, emit)) as unknown as Record<string, unknown>,
    (err) =>
